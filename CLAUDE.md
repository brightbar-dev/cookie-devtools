...
- **Withheld site access**: a user can set the extension's Site access to "On click" or specific sites. `chrome.cookies` ignores `activeTab` (measured 2026-09-15: after a toolbar click `getAll` returns `[]` and `set` throws "No host permissions for cookies"), so `loadCookies` checks `permissions.contains({origins: [siteAccessPattern(url)]})` and the empty state says the site is not readable and offers `permissions.request` from the click; `permissions.onAdded`/`onRemoved` reload the list.
